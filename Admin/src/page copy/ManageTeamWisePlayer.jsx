...
const ManageTeamWisePlayer = () => {
    ...
    useEffect(() => {
        const selectedPlayer = selectedPlayers.map(item => item.id)
    ...
    const fetchAllData = async (team) => {
        ...
        if (response && Object.keys(response.data.data).length > 0) {
            ...
            console.log("L288", team, response.data.data[0]);
            // let arr = response.data.data.sort().reverse().map((item, index) => {
            let arr = response?.data?.data[0].playerdata
                .map((item, index) => {
                    return {
                        sl: index + 1,
