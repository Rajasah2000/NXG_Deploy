...
const MatchList = () => {
    ...
    const fetchAllData = async () => {
        ...
        if (response && Object.keys(response.data.data).length > 0) {
            ...
            let arr = response.data.data.sort().reverse().map((item, index) => {
                console.log("playingelevendta", item);
                return {
                    sl: index + 1,
                        ...
                        <div style={{ display: "flex", flexDirection: "coloum" }}>
                            {
                                !item?.isToss_completed ? (
                                    ...
                                    <>
                                        <div>Won by <strong>{item?.winnerTeamNme}</strong> and decided to <strong>{item?.toss_winner_playBy}</strong></div>
                                    </>
                                )
                            }
                        ...
                        <div style={{ display: "flex", flexDirection: "coloum" }}>
                            {
                                !item?.isMatch_started && item?.isToss_completed && item?.playingelevendta.length === 2 ? (
                                    ...
                                    <>
                                        {
                                            item?.isToss_completed && item?.playingelevendta.length === 2 ? (
                                                <>
                                                    <button
                    ...
                    matchUpdate: (
                        <div style={{ display: "flex", flexDirection: "coloum" }}>
                            {
                                item?.isMatch_started && item?.isToss_completed ? (
                                    <>
                                        {/* <Link to={`/overupdate/${item._id}`}>Match Update</Link> */}
                                        <Link to={`/overupdate/${item._id}`} class="btn btn-success">
                                            {/* <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span><br /> */}
                                            Match Update
                                        </Link>
                                    </>
                                ) : (
                                    <>
                                        Match yet not started
                                    </>
                                )
                            }
    ...
    const handleTeamPlayersData = async (e) => {
        ...
        console.log("L587", selectedPlayers)
        ...
        // console.log("inputPlayingEleven742", inputPlayingEleven)
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div class="col-md-12">
                                                                        <div class="form-floating mb-3">
                                                                            ...
                                                                            <label for="wicket_keeperID">Wicket Keeper:</label>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </>
